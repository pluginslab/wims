...
`assets/wims.png` is a rasterised copy of `assets/wims.svg`, used as the social
preview image where SVG is not supported. It is regenerated by hand on macOS,
since it depends on QuickLook:

```bash
qlmanage -t -s 1744 -o /tmp assets/wims.svg
ffmpeg -y -i /tmp/wims.svg.png -vf "crop=1744:1324:0:0" assets/wims.png
```

Only refresh it when the SVG changes meaningfully.
