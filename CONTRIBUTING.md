...
`assets/*.png` are rasterised copies of the SVGs, for the places SVG is not
supported: social previews, and WordPress, which blocks SVG uploads. Regenerate
them with headless Chrome, which honours the SVG's declared size exactly:

```bash
CHROME="/Applications/Google Chrome.app/Contents/MacOS/Google Chrome"
"$CHROME" --headless --disable-gpu --force-device-scale-factor=2 \
  --window-size=872,602 --screenshot="$PWD/assets/wims.png" \
  "file://$PWD/assets/wims.svg"
```

Use each SVG's own `width`/`height` for `--window-size`. Do not use macOS
`qlmanage` for this: it pads the output onto a square canvas and scales in a way
that silently clips the last row or two.

Only refresh them when the SVGs change meaningfully.
