...
The installer builds the TUI, links `wims-tui` into `~/.local/bin`, and adds a
small `wims` function to your shell rc file (zsh, bash and fish are detected).
Open a new shell and run `wims`.

Reverse all of it with `./install.sh --uninstall`.
...
Typing filters on the title and the folder path — *"I know roughly what it was
called"*.
